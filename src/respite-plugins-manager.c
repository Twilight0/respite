...
#define RESPITE_PLUGIN_EXT = ".desktop"
...
void respite_plugins_manager_pref_response_cb(GtkDialog *dialog,
                                                    gint reponse_id,
                                                    PrefData *pref);

void respite_plugins_manager_cell_toggled_cb(GtkCellRendererToggle *cell_renderer,
                                                    gchar *path,
                                                    PrefData *pref);

void respite_plugins_manager_tree_cursor_changed_cb(GtkTreeView *view,
                                                    PrefData *pref);

void respite_plugins_manager_show_configure(GtkButton *button,
                                                    PrefData *pref);

void respite_plugins_manager_show_about(GtkButton *button,
                                                    PrefData *pref);

static void respite_plugins_manager_finalize(GObject *object);
static void respite_plugins_manager_set_property(GObject *object,
                                                    guint prop_id,
                                                    const GValue *value,
                                                    GParamSpec *pspec);

struct RespitePluginsManagerPrivate {
    GtkWidget *list_nt;
...
static gpointer respite_plugins_manager_object = NULL;

G_DEFINE_TYPE_WITH_PRIVATE(RespitePluginsManager, respite_plugins_manager, G_TYPE_OBJECT)
...
void respite_plugins_manager_pref_response_cb(GtkDialog *dialog, gint reponse_id, PrefData *pref) {
    GtkTreeIter iter;
    gboolean    valid;
...
static void
respite_plugins_manager_get_selected_module_data(PrefData *pref,
...
void respite_plugins_manager_show_configure(GtkButton *button, PrefData *pref) {
    RespiteProviderModule *module;
    ParolePluginInfo     *info;

    respite_plugins_manager_get_selected_module_data(pref, &module, &info);

    if ( G_UNLIKELY (!module) )
        return;

    respite_provider_plugin_configure(RESPITE_PROVIDER_PLUGIN(module), pref->window);
}

void respite_plugins_manager_show_about(GtkButton *button, PrefData *pref) {
    RespiteProviderModule *module;
    ParolePluginInfo     *info;
    ...
    gtk_about_dialog_set_logo_icon_name(about, "respite-extension");
...
static void
respite_plugins_manager_save_rc(RespitePluginsManager *manager, gchar *filename, gboolean active) {
    gchar **saved_plugins;
    gchar **plugins_rc;
    gchar  *basename;
    guint num = 0, i, count = 0;

    saved_plugins = respite_conf_read_entry_list(RESPITE_CONF(manager->priv->conf), "plugins");
    ...
    if ( active ) {
        ...
        respite_conf_write_entry_list(RESPITE_CONF(manager->priv->conf), "plugins", plugins_rc);
    } else {
...
void
respite_plugins_manager_cell_toggled_cb(GtkCellRendererToggle *cell_renderer,
                                       gchar *path_str,
                                       PrefData *pref) {
    ...
    if (gtk_tree_model_get_iter(GTK_TREE_MODEL(pref->store), &iter, path)) {
        ...
        if ( pref->manager->priv->load_plugins ) {
            if ( active ) {
                respite_provider_module_use(module);
                if (!respite_provider_module_new_plugin(module)) {
                    // If plugin loading fails...
                    respite_dialog_error(GTK_WINDOW(pref->window),
                        _("Plugin failed to load"),
                        _("Please check your installation"));
                    respite_provider_module_free_plugin(module);
                    respite_provider_module_unuse(module);
                    active = FALSE;
                }
            } else {
                respite_provider_module_free_plugin(module);
        ...
        respite_plugins_manager_save_rc(pref->manager, G_TYPE_MODULE(module)->name, active);
    }

    gtk_tree_path_free(path);
}

void respite_plugins_manager_tree_cursor_changed_cb(GtkTreeView *view, PrefData *pref) {
    RespiteProviderModule *module;
    ParolePluginInfo *info;
    ...
    if (respite_provider_module_get_is_active(module)) {
        configurable = respite_provider_plugin_get_is_configurable(RESPITE_PROVIDER_PLUGIN(module));
    }
...
static void
respite_plugins_manager_unload_all(gpointer data, gpointer user_data) {
    RespiteProviderModule *module;

    module = RESPITE_PROVIDER_MODULE(data);
    if (respite_provider_module_get_is_active(module)) {
        respite_provider_module_free_plugin(module);
...
respite_plugins_manager_get_plugin_info(const gchar *desktop_file) {
    ParolePluginInfo *info;
    GKeyFile *file;
    GError *error = NULL;
...
static void
respite_plugins_manager_show_plugins_pref(GtkWidget *widget, RespitePluginsManager *manager) {
    GtkBuilder *builder;
    GtkTreeSelection *sel;
    GtkTreePath *path;
    GtkTreeIter iter;
    ...
    if ( manager->priv->array->len == 0 ) {
        respite_dialog_info(GTK_WINDOW(manager->priv->main_window),
        _("No installed plugins found on this system"),
        _("Please check your installation."));
        return;
    }

    builder = respite_builder_new_from_resource("/org/gnome/respite/plugins.ui");
    ...
    for (i = 0; i < manager->priv->array->len; i++) {
        ...
        info = respite_plugins_manager_get_plugin_info(module->desktop_file);
        desc = g_strdup_printf("<b>%s</b>\n%s", info->name, info->desc);

        gtk_list_store_append(pref->store, &iter);
        gtk_list_store_set(pref->store, &iter,
                            COL_ACTIVE, respite_provider_module_get_is_active(module),
                            COL_PLUGIN, desc,
                            COL_MODULE, module,
                            COL_INFO, info,
                            -1);
...
static void
respite_plugins_manager_set_show_tabs(GtkNotebook *nt) {
    gint npages;
...
static void
respite_plugins_manager_page_added_cb(GtkContainer *container, GtkWidget *widget, gpointer data) {
    respite_plugins_manager_set_show_tabs(GTK_NOTEBOOK(container));
}

static void
respite_plugins_manager_page_removed_cb(GtkContainer *container, GtkWidget *widget, gpointer data) {
    respite_plugins_manager_set_show_tabs(GTK_NOTEBOOK(container));
}
...
static void respite_plugins_manager_set_property(GObject *object,
    ...
    manager = RESPITE_PLUGINS_MANAGER(object);
...
static void respite_plugins_manager_get_property(GObject *object,
                                                guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
    RespitePluginsManager *manager;
...
static gchar *
respite_plugins_manager_get_module_name(const gchar *desktop_file) {
    GKeyFile *file;
    GError *error = NULL;
...
static void
respite_plugins_manager_load_plugins(RespitePluginsManager *manager) {
    gchar **plugins_rc;
    guint len = 0, i, j;

    plugins_rc = respite_conf_read_entry_list(RESPITE_CONF(manager->priv->conf), "plugins");
    ...
    for (j = 0; j < manager->priv->array->len; j++) {
        ...
        for (i = 0; i < len; i++) {
            if ( g_strcmp0 (plugins_rc[i], module->name) == 0 ||
                 g_strcmp0(plugins_rc[i], RESPITE_PROVIDER_MODULE(module)->library_name) == 0 ) {
                TRACE("Loading plugin :%s", module->name);
                if ( !respite_provider_module_use (RESPITE_PROVIDER_MODULE(module)) ) {
                    respite_plugins_manager_save_rc(manager, module->name, FALSE);
                    g_ptr_array_remove(manager->priv->array, module);
                    g_object_unref(module);
                } else {
                    respite_provider_module_new_plugin(RESPITE_PROVIDER_MODULE(module));
                }
                break;
            }
        }
...
static void
respite_plugins_manager_class_init(RespitePluginsManagerClass *klass) {
    GObjectClass *object_class = G_OBJECT_CLASS(klass);

    object_class->finalize     = respite_plugins_manager_finalize;
    object_class->set_property = respite_plugins_manager_set_property;
    object_class->get_property = respite_plugins_manager_get_property;
...
static void
respite_plugins_manager_init(RespitePluginsManager *manager) {
    GtkBuilder *builder;

    manager->priv = respite_plugins_manager_get_instance_private(manager);
    ...
    manager->priv->conf = respite_conf_new();

    g_signal_connect(manager->priv->list_nt, "page-added",
                        G_CALLBACK(respite_plugins_manager_page_added_cb), NULL);

    g_signal_connect(manager->priv->list_nt, "page-removed",
                        G_CALLBACK(respite_plugins_manager_page_removed_cb), NULL);

    g_signal_connect(gtk_builder_get_object(builder, "plugins-menu-item"), "activate",
                        G_CALLBACK(respite_plugins_manager_show_plugins_pref), manager);

    respite_plugins_manager_set_show_tabs(GTK_NOTEBOOK(manager->priv->list_nt));

    g_object_unref(builder);
}

static void
respite_plugins_manager_finalize(GObject *object) {
    RespitePluginsManager *manager;

    manager = RESPITE_PLUGINS_MANAGER(object);

    g_ptr_array_foreach(manager->priv->array,
                            (GFunc)respite_plugins_manager_unload_all, NULL);
    g_ptr_array_free(manager->priv->array, TRUE);

    G_OBJECT_CLASS(respite_plugins_manager_parent_class)->finalize(object);
}

RespitePluginsManager *
respite_plugins_manager_new(gboolean load_plugins) {
    if ( G_LIKELY(respite_plugins_manager_object != NULL) ) {
        g_object_ref(respite_plugins_manager_object);
    } else {
        respite_plugins_manager_object = g_object_new(RESPITE_TYPE_PLUGINS_MANAGER,
                                                        "load-plugins", load_plugins,
                                                        NULL);
        g_object_add_weak_pointer(respite_plugins_manager_object, &respite_plugins_manager_object);
    }

    return RESPITE_PLUGINS_MANAGER (respite_plugins_manager_object);
}

RespitePluginsManager *
respite_plugins_manager_get(void) {
    if ( G_UNLIKELY (respite_plugins_manager_object == NULL ) )
...
void respite_plugins_manager_load(RespitePluginsManager *manager) {
    ...
    dir = g_dir_open(RESPITE_PLUGINS_DATA_DIR, 0, &error);
    ...
    while ((name = g_dir_read_name(dir))) {
        ...
        desktop_file = g_build_filename(RESPITE_PLUGINS_DATA_DIR, name, NULL);
        library_name = respite_plugins_manager_get_module_name(desktop_file);

        if ( library_name ) {
            RespiteProviderModule *module;
            gchar *library_path;

            library_path = g_build_filename(RESPITE_PLUGINS_DIR, library_name, NULL);
            ...
            module = respite_provider_module_new(library_path, desktop_file, library_name);
    ...
    if ( manager->priv->load_plugins )
        respite_plugins_manager_load_plugins(manager);
}

void
respite_plugins_manager_pack(RespitePluginsManager *manager,
