...
static void       respite_gst_play_file_internal(RespiteGst *gst);

static void       respite_gst_change_state(RespiteGst *gst,
                                                          GstState new);

static void       respite_gst_terminate_internal(RespiteGst *gst);

static GdkPixbuf *respite_gst_tag_list_get_cover_external(RespiteGst *gst);

static GdkPixbuf *respite_gst_tag_list_get_cover_embedded(RespiteGst *gst,
                                                          GstTagList *tag_list);

static GdkPixbuf *respite_gst_tag_list_get_cover(RespiteGst *gst,
                                                          GstTagList *tag_list);
...
struct RespiteGstPrivate {
     ...
     */
    RespiteGstVideoSink  image_sink;
...
static gpointer respite_gst_object = NULL;
...
static void
respite_gst_finalize(GObject *object) {
    RespiteGst *gst;

    gst = RESPITE_GST(object);
    ...
    G_OBJECT_CLASS(respite_gst_parent_class)->finalize(object);
}

static gboolean
respite_gst_configure_event_cb(GtkWidget *widget, GdkEventConfigure *ev, RespiteGst *gst) {
    return FALSE;
}

static gboolean
respite_gst_parent_expose_event(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
...
static void
respite_gst_realize(GtkWidget *widget) {
    RespiteGst *gst;
    ...
    g_signal_connect(gtk_widget_get_toplevel(widget), "configure-event",
                         G_CALLBACK(respite_gst_configure_event_cb), gst);
...
static void
respite_gst_show(GtkWidget *widget) {
    if ( gtk_widget_get_window(widget) )
        gdk_window_show(gtk_widget_get_window(widget));

    if ( GTK_WIDGET_CLASS (respite_gst_parent_class)->show )
        GTK_WIDGET_CLASS(respite_gst_parent_class)->show(widget);
}

static void
respite_gst_get_video_output_size(RespiteGst *gst, gint *ret_w, gint *ret_h) {
    /*
...
static void
respite_gst_size_allocate(GtkWidget *widget, GtkAllocation *allocation) {
    ...
    if (gtk_widget_get_realized(widget)) {
        ...
        respite_gst_get_video_output_size(RESPITE_GST(widget), &w, &h);
...
static void
respite_gst_set_video_color_balance(RespiteGst *gst) {
    GstElement *video_sink;
...
static void
respite_gst_set_video_overlay(RespiteGst *gst) {
    GstElement *video_sink;
...
static void
respite_gst_query_capabilities(RespiteGst *gst) {
    GstQuery *query;
    gboolean seekable;
...
static gboolean
respite_gst_tick_timeout(gpointer data) {
    RespiteGst *gst;
    ...
    if (gst->priv->state == GST_STATE_PLAYING) {
        if (duration != 0 && seekable == FALSE) {
            respite_gst_query_capabilities(gst);
        }
...
static void
respite_gst_tick(RespiteGst *gst) {
    gboolean live;
    ...
    if (gst->priv->state >= GST_STATE_PAUSED && !live) {
        ...
        gst->priv->tick_id = g_timeout_add(250, (GSourceFunc)respite_gst_tick_timeout, gst);
    } else {
        g_clear_handle_id(&gst->priv->tick_id, g_source_remove);
    }
}
...
static void
respite_gst_query_duration(RespiteGst *gst) {
...
static void
respite_gst_set_subtitle_font(RespiteGst *gst) {
    gchar *font;
...
static void
respite_gst_set_subtitle_encoding(RespiteGst *gst) {
    gchar *encoding;
...
static void
respite_gst_load_subtitle(RespiteGst *gst) {
    RespiteMediaType type;
    gchar *uri;
    gchar *sub;
    gchar *sub_uri;
...
static void
respite_gst_get_pad_capabilities(GObject *object, GParamSpec *pspec, RespiteGst *gst) {
    GstPad *pad;
    GstStructure *st;
    GtkAllocation allocation = { 0 };
    gint width;
    ...
    if ( st ) {
        ...
        gtk_widget_get_allocation(GTK_WIDGET(gst), &allocation);
        respite_gst_size_allocate(GTK_WIDGET(gst), &allocation);
    }
    gst_caps_unref(caps);
}

static void
respite_gst_query_info(RespiteGst *gst) {
    ...
    if (n_video > 0) {
        ...
        if (videopad) {
            ...
            if ((caps = gst_pad_get_current_caps (videopad))) {
                respite_gst_get_pad_capabilities(G_OBJECT(videopad), NULL, gst);
                gst_caps_unref(caps);
            }

            g_signal_connect(videopad, "notify::caps",
                      G_CALLBACK(respite_gst_get_pad_capabilities),
                      gst);
...
static void
respite_gst_update_stream_info(RespiteGst *gst) {
    TRACE("Updating stream info");
    respite_gst_query_info(gst);
}

static void
respite_gst_update_vis(RespiteGst *gst) {
    gchar *vis_name;
    gint flags;
...
static void
respite_gst_evaluate_state (RespiteGst *gst, GstState old, GstState new, GstState pending) {
    GtkAllocation allocation = { 0 };
    TRACE ("State change new %i old %i pending %i", new, old, pending);

    gst->priv->state = new;

    respite_gst_tick(gst);

    if ( gst->priv->target == new ) {
        gtk_widget_queue_draw(GTK_WIDGET(gst));
        respite_window_normal_cursor(gtk_widget_get_window(GTK_WIDGET(gst)));
        if ( gst->priv->state_change_id != 0 ) {
            ...
            if (respite_gst_get_current_stream_type(gst) == PAROLE_MEDIA_TYPE_DVD) {
                gst->priv->state_change_id = g_timeout_add_seconds(5,
                                                (GSourceFunc) respite_gst_dvd_menu_timeout,
                                                gst);
            }
        }
    }
    ...
    switch (gst->priv->state) {
        case GST_STATE_PLAYING:
        {
            ...
            respite_gst_query_capabilities(gst);
            respite_gst_query_info(gst);
            respite_gst_query_duration(gst);
        ...
        case GST_STATE_PAUSED:
        {
            if ( gst->priv->target == GST_STATE_PLAYING ) {
                if ( gst->priv->update_color_balance ) {
                    respite_gst_set_video_color_balance(gst);
                }
            }
        ...
        case GST_STATE_READY:
        {
            ...
            if (gst->priv->target == GST_STATE_PLAYING && pending < GST_STATE_PAUSED) {
                respite_gst_play_file_internal(gst);
            } else if (gst->priv->target == GST_STATE_PAUSED) {
                respite_gst_change_state(gst, GST_STATE_PAUSED);
...
static void
respite_gst_element_message_sync(GstBus *bus, GstMessage *message, RespiteGst *gst) {
...
static GdkPixbuf *
respite_gst_buffer_to_pixbuf(GstBuffer *buffer) {
...
GdkPixbuf *
respite_gst_tag_list_get_cover_external(RespiteGst *gst) {
    GdkPixbuf *pixbuf;
    GError *err = NULL;
    gchar *uri;
    gchar *filename;
...
GdkPixbuf *
respite_gst_tag_list_get_cover_embedded(RespiteGst *gst, GstTagList *tag_list) {
    GdkPixbuf *pixbuf;
    ...
    if (cover_value) {
        ...
        pixbuf = respite_gst_buffer_to_pixbuf(gst_sample_get_buffer(sample));
        return pixbuf;
    }
    return NULL;
}
...
GdkPixbuf *
respite_gst_tag_list_get_cover(RespiteGst *gst, GstTagList *tag_list) {
    GdkPixbuf *pixbuf;
    GdkPixbuf *scaled;
    gint height, width;
    gfloat multiplier;
    ...
    pixbuf = respite_gst_tag_list_get_cover_external(gst);
    if (!pixbuf) {
        pixbuf = respite_gst_tag_list_get_cover_embedded(gst, tag_list);
        if (!pixbuf)
            return NULL;
    }
...
static void
respite_gst_get_meta_data_dvd(RespiteGst *gst) {
    gint n_chapters;
...
static void
respite_gst_get_meta_data_cdda(RespiteGst *gst, GstTagList *tag) {
    guint num_tracks;
    guint track;
    ...
    if (gst_tag_list_get_uint (tag, GST_TAG_TRACK_NUMBER, &track) &&
        ...
        respite_stream_set_image(G_OBJECT(gst->priv->stream), NULL);
        g_object_set(G_OBJECT(gst->priv->stream),
                     "tag-available", FALSE,
                     NULL);
...
static void
respite_gst_get_meta_data_file(RespiteGst *gst, GstTagList *tag) {
    gchar *str;
    GDate *date;
    guint integer;
    ...
    g_object_get(G_OBJECT(gst->priv->stream),
                 ...
                 NULL);
    if (!has_artwork) {
        pixbuf = respite_gst_tag_list_get_cover(gst, tag);
        if (pixbuf) {
            respite_stream_set_image(G_OBJECT(gst->priv->stream), pixbuf);
            g_object_unref(pixbuf);
        }
    }
...
static void
respite_gst_get_meta_data_unknown(RespiteGst *gst) {
    g_object_set(G_OBJECT(gst->priv->stream),
                 "title", NULL,
                 "artist", NULL,
                 "year", NULL,
...
static void
respite_gst_get_meta_data(RespiteGst *gst, GstTagList *tag) {
    ...
    switch ( media_type ) {
        ...
        case PAROLE_MEDIA_TYPE_REMOTE:
            respite_gst_get_meta_data_file(gst, tag);
            break;
        case PAROLE_MEDIA_TYPE_CDDA:
            respite_gst_get_meta_data_cdda(gst, tag);
            break;
        case PAROLE_MEDIA_TYPE_DVD:
            respite_gst_get_meta_data_dvd(gst);
            break;
        case PAROLE_MEDIA_TYPE_UNKNOWN:
            respite_gst_get_meta_data_unknown(gst);
            break;
        default:
            break;
    }
...
static void
respite_gst_application_message(RespiteGst *gst, GstMessage *msg) {
    GtkAllocation allocation = { 0 };
    if (gst_message_has_name(msg, "notify-streaminfo")) {
        respite_gst_update_stream_info(gst);
...
static void
respite_gst_missing_codec_helper_dialog(void) {
...
/**
 * respite_gst_install_plugins_result_func:
 ...
 **/
static void
respite_gst_install_plugins_result_func(GstInstallPluginsReturn result, gpointer user_data) {
    switch (result) {
        case GST_INSTALL_PLUGINS_SUCCESS:
            g_debug("Finished plugin install.\n");
            gst_update_registry();
            respite_gst_resume(RESPITE_GST(user_data));
            break;
...
static GtkDialog*
respite_gst_missing_codec_dialog(RespiteGst *gst, GstMessage *msg) {
    GtkMessageDialog *dialog;
    gchar*     desc;
...
static gboolean
respite_gst_bus_event(GstBus *bus, GstMessage *msg, gpointer data) {
    RespiteGst                *gst;
    GtkDialog                *dialog;
    gchar*                    details[2];
    ...
    case GST_MESSAGE_STATE_CHANGED:
    {
        ...
        if (GST_MESSAGE_SRC(msg) == GST_OBJECT(gst->priv->playbin)) {
            respite_gst_evaluate_state (gst, old, new, pending);
        }
        break;
    }
    ...
    case GST_MESSAGE_TAG:
    {
        if ( gst->priv->enable_tags ) {
            ...
            respite_gst_get_meta_data(gst, tag_list);
            gst_tag_list_free(tag_list);
        }
        break;
    }
    case GST_MESSAGE_APPLICATION:
        respite_gst_application_message(gst, msg);
        break;
    ...
    case GST_MESSAGE_ELEMENT:
        if (gst_is_missing_plugin_message(msg)) {
            ...
            dialog = respite_gst_missing_codec_dialog(gst, msg);
            ...
            if ( response == GTK_RESPONSE_ACCEPT ) {
                ...
                result = gst_install_plugins_async((const gchar * const *) details,
                    ctx, respite_gst_install_plugins_result_func, gst);

                if (result == GST_INSTALL_PLUGINS_HELPER_MISSING)
                    respite_gst_missing_codec_helper_dialog();
...
static void
respite_gst_change_state (RespiteGst *gst, GstState new) {
    ...
    switch (ret) {
        case GST_STATE_CHANGE_SUCCESS:
            respite_gst_evaluate_state(gst,
...
static void
respite_gst_source_notify_cb(GObject *obj, GParamSpec *pspec, RespiteGst *gst) {
    GObject *source;
...
static void
respite_gst_play_file_internal(RespiteGst *gst) {
    gchar *uri;
    ...
    respite_gst_load_subtitle(gst);
    respite_gst_change_state(gst, GST_STATE_PLAYING);
    g_free(uri);
}

void
respite_gst_send_navigation_command(RespiteGst *gst, gint command) {
    GstNavigation *nav;
...
static gboolean
respite_gst_motion_notify_event(GtkWidget *widget, GdkEventMotion *ev) {
    gboolean ret = FALSE;

    if (GTK_WIDGET_CLASS (respite_gst_parent_class)->motion_notify_event)
        ret |= GTK_WIDGET_CLASS(respite_gst_parent_class)->motion_notify_event(widget, ev);

    return ret;
}

static gboolean
respite_gst_button_press_event(GtkWidget *widget, GdkEventButton *ev) {
    RespiteGst *gst;
    GstNavigation *nav;
    ...
    if (GTK_WIDGET_CLASS (respite_gst_parent_class)->button_press_event)
        ret |= GTK_WIDGET_CLASS(respite_gst_parent_class)->button_press_event(widget, ev);

    return ret;
}

static gboolean
respite_gst_button_release_event(GtkWidget *widget, GdkEventButton *ev) {
    RespiteGst *gst;
    GstNavigation *nav;
    gboolean playing_video;
    gboolean ret = FALSE;
    ...
    if (GTK_WIDGET_CLASS (respite_gst_parent_class)->button_release_event)
        ret |= GTK_WIDGET_CLASS(respite_gst_parent_class)->button_release_event(widget, ev);

    return ret;
}

static void
respite_gst_seek_by_format(RespiteGst *gst, GstFormat format, gint step) {
    gint64 val = 1;
...
static void
respite_gst_change_dvd_chapter(RespiteGst *gst, gint level) {
    GstFormat format = gst_format_get_by_nick("chapter");

    respite_gst_seek_by_format(gst, format, level);
}

static void
respite_gst_change_cdda_track(RespiteGst *gst, gint level) {
    GstFormat format = gst_format_get_by_nick("track");

    respite_gst_seek_by_format(gst, format, level);
}

RespiteMediaType respite_gst_get_current_stream_type(RespiteGst *gst) {
    RespiteMediaType type;
    g_object_get(G_OBJECT(gst->priv->stream),
                  "media-type", &type,
                  NULL);
...
static gboolean
respite_gst_check_state_change_timeout(gpointer data) {
    RespiteGst *gst;
    GtkWidget *dialog;
    ...
    if ( gst->priv->state != gst->priv->target ) {
        ...
        if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_CANCEL) {
            respite_gst_terminate_internal(gst);
...
static gboolean
respite_gst_dvd_menu_timeout(gpointer data) {
    RespiteGst *gst;
    gint64     duration;

    gst = RESPITE_GST(data);

    duration = respite_gst_get_stream_duration(gst);

    if ( duration == 0 ) {
        respite_gst_send_navigation_command(gst, GST_NAVIGATION_COMMAND_DVD_MENU);
        gst->priv->state_change_id = 0;
        return TRUE;
    }
    return FALSE;
...
static gboolean
respite_gst_check_dvd_state_change_timeout(gpointer data) {
    RespiteGst *gst;
...
static void
respite_gst_terminate_internal(RespiteGst *gst) {
    gboolean playing_video;
...
static void
respite_gst_about_to_finish_cb(GstElement *elm, gpointer data) {
    RespiteGst *gst;
...
static void
respite_gst_conf_notify_cb(GObject *object, GParamSpec *spec, RespiteGst *gst) {
    GtkAllocation allocation = { 0 };
    if ( g_strcmp0("vis-enabled", spec->name) == 0 || g_strcmp0("vis-name", spec->name) == 0 ) {
        ...
        respite_gst_set_subtitle_font(gst);
    } else if (g_strcmp0("subtitle-encoding", spec->name) == 0) {
        respite_gst_set_subtitle_encoding(gst);
...
static void
respite_gst_conf_notify_volume_cb(GObject *conf, GParamSpec *pspec, RespiteGst *gst) {
    gint volume;
    ...
    respite_gst_set_volume (gst, (double)(volume / 100.0));
}

static void respite_gst_get_property(GObject *object,
                                        guint prop_id,
                                        GValue *value,
                                        GParamSpec *pspec) {
    RespiteGst *gst;
...
static void respite_gst_set_property(GObject *object,
                                        guint prop_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
    RespiteGst *gst;
    ...
    switch (prop_id) {
        case PROP_ENABLE_TAGS:
            ...
            break;
        case PROP_VOLUME:
            respite_gst_set_volume(gst, g_value_get_double(value));
            break;
        case PROP_CONF_OBJ:
            ...
            if (gst->priv->conf) {
                ...
                G_CALLBACK(respite_gst_conf_notify_cb), gst);
                g_signal_connect(G_OBJECT(gst->priv->conf), "notify::volume",
                G_CALLBACK(respite_gst_conf_notify_volume_cb), gst);
            }
            break;
        default:
            G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
...
static gboolean
respite_notify_volume_idle_cb(RespiteGst *gst) {
    gdouble vol;
...
static void
respite_notify_volume_cb(GObject        *object,
                         GParamSpec     *pspec,
                         RespiteGst      *gst) {
    g_idle_add((GSourceFunc) respite_notify_volume_idle_cb, gst);
}

static void
respite_gst_show_error(GtkWindow *window, GError *error) {
    GtkWidget *dialog;
    gchar *message;
    dialog = gtk_message_dialog_new(NULL,
                                     ...
                                     GTK_BUTTONS_CLOSE,
                                     _("GStreamer Error"));
    message = g_strdup_printf("%s\n%s", error->message, _("Respite Media Player cannot start."));
...
static void
respite_gst_constructed(GObject *object) {
    RespiteGst *gst;
    ...
    if (G_UNLIKELY(gst->priv->playbin == NULL)) {
        ...
        respite_gst_show_error(GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(gst))),
                                error);
        g_error_free(error);
    ...
    g_signal_connect(gst->priv->bus, "message",
                        G_CALLBACK(respite_gst_bus_event), gst);
    ...
    g_signal_connect(gst->priv->bus, "sync-message::element",
                      G_CALLBACK(respite_gst_element_message_sync), gst);

    g_signal_connect(gst->priv->playbin, "notify::source",
                      G_CALLBACK(respite_gst_source_notify_cb), gst);

    g_signal_connect(gst->priv->playbin, "notify::volume",
                      G_CALLBACK(respite_notify_volume_cb), gst);


    g_signal_connect(gst->priv->playbin, "about-to-finish",
                      G_CALLBACK(respite_gst_about_to_finish_cb), gst);
    ...
    G_OBJECT_CLASS(respite_gst_parent_class)->constructed(object);
}

static void
respite_gst_class_init(RespiteGstClass *klass) {
    GObjectClass *object_class = G_OBJECT_CLASS(klass);
    GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

    object_class->finalize = respite_gst_finalize;
    object_class->constructed = respite_gst_constructed;
    object_class->set_property = respite_gst_set_property;
    ...
    widget_class->motion_notify_event = respite_gst_motion_notify_event;
    widget_class->button_press_event = respite_gst_button_press_event;
    widget_class->button_release_event = respite_gst_button_release_event;

    signals[MEDIA_STATE] =
        g_signal_new("media-state",
                        RESPITE_TYPE_GST,
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET(RespiteGstClass, media_state),
                        NULL, NULL,
    ...
    signals[MEDIA_PROGRESSED] =
        g_signal_new("media-progressed",
                        RESPITE_TYPE_GST,
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET(RespiteGstClass, media_progressed),
                        NULL, NULL,
    ...
    signals[MEDIA_SEEKED] =
        g_signal_new("media-seeked",
                        RESPITE_TYPE_GST,
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET(RespiteGstClass, media_seeked),
                        NULL, NULL,
    ...
    signals[MEDIA_TAG] =
        g_signal_new("media-tag",
                        RESPITE_TYPE_GST,
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET(RespiteGstClass, media_tag),
                        NULL, NULL,
    ...
    signals[BUFFERING] =
        g_signal_new("buffering",
                        RESPITE_TYPE_GST,
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET(RespiteGstClass, buffering),
                        NULL, NULL,
    ...
    signals[ERROR] =
        g_signal_new("error",
                        RESPITE_TYPE_GST,
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET(RespiteGstClass, error),
                        NULL, NULL,
    ...
    signals[DVD_CHAPTER_CHANGE] =
        g_signal_new("dvd-chapter-change",
                        RESPITE_TYPE_GST,
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET(RespiteGstClass, dvd_chapter_change),
                        NULL, NULL,
    ...
    signals[DVD_CHAPTER_COUNT_CHANGE] =
        g_signal_new("dvd-chapter-count-change",
                        RESPITE_TYPE_GST,
                        G_SIGNAL_RUN_LAST,
                        G_STRUCT_OFFSET(RespiteGstClass, dvd_chapter_count_change),
                        NULL, NULL,
                        g_cclosure_marshal_VOID__INT,
                        G_TYPE_NONE, 1, G_TYPE_INT);
...
static void
respite_gst_init(RespiteGst *gst) {
    gst->priv = respite_gst_get_instance_private(gst);

    gst->priv->state = GST_STATE_VOID_PENDING;
    gst->priv->target = GST_STATE_VOID_PENDING;
    gst->priv->media_state = PAROLE_STATE_STOPPED;
    gst->priv->aspect_ratio = RESPITE_ASPECT_RATIO_NONE;
    g_mutex_init(&gst->priv->lock);
    gst->priv->stream = respite_stream_new();
    gst->priv->tick_id = 0;
    gst->priv->update_vis = FALSE;
    gst->priv->buffering = FALSE;
    gst->priv->seeking = FALSE;
...
GtkWidget *
respite_gst_new(gpointer conf_obj) {
    respite_gst_object = g_object_new(RESPITE_TYPE_GST,
                                      "conf-object", conf_obj,
                                      NULL);

    g_object_add_weak_pointer(respite_gst_object, &respite_gst_object);

    return GTK_WIDGET (respite_gst_object);
}

GtkWidget *respite_gst_get(void) {
    if ( G_LIKELY(respite_gst_object != NULL ) ) {
    /*
     * Don't increase the reference count of this object as
     * we need it to be destroyed immediately when the main
     * window is destroyed.
     */
    // g_object_ref (respite_gst_object);
    } else {
        respite_gst_object = g_object_new(RESPITE_TYPE_GST, NULL);
        g_object_add_weak_pointer(respite_gst_object, &respite_gst_object);
    }
...
static gboolean
respite_gst_play_idle(gpointer data) {
    RespiteGst *gst;
    ...
    if ( gst->priv->state < GST_STATE_PAUSED )
        ...
        respite_gst_change_state(gst, GST_STATE_READY);

    return FALSE;
}

void respite_gst_set_custom_subtitles(RespiteGst *gst, const gchar* sub_file) {
    if ( sub_file == NULL ) {
        gst->priv->use_custom_subtitles = FALSE;
        gst->priv->custom_subtitles = NULL;
    } else {
    ...
    if ( gst->priv->state_change_id == 0 ) {
        if (respite_gst_get_current_stream_type(gst) == PAROLE_MEDIA_TYPE_DVD) {
            gst->priv->state_change_id = g_timeout_add_seconds(3,
                                        (GSourceFunc) respite_gst_check_dvd_state_change_timeout,
                                        gst);
        } else {
            gst->priv->state_change_id = g_timeout_add_seconds(20,
                                        (GSourceFunc) respite_gst_check_state_change_timeout,
                                        gst);
        }
    }
...
void respite_gst_play_device_uri(RespiteGst *gst, const gchar *uri, const gchar *device) {
    ...
    respite_gst_play_uri(gst, local_uri, NULL);
}
...
static gboolean
respite_gst_stop_idle(gpointer data) {
    RespiteGst *gst;
...
void respite_gst_stop(RespiteGst *gst) {
    ...
    g_idle_add((GSourceFunc) respite_gst_stop_idle, gst);
}
...
void respite_gst_shutdown(RespiteGst *gst) {
     ...
     * Seems to be some last-second threading issues. When respite is closed,
     * these signals are disconnected before the process ends. With recent
     * library versions, respite crashes at this point. Since the signals will
...
void respite_gst_seek(RespiteGst *gst, gdouble seek) {
    TRACE("Seeking");
    g_warn_if_fail(gst_element_seek(gst->priv->playbin,
                        1.0,
                        GST_FORMAT_TIME,
                        GST_SEEK_FLAG_ACCURATE | GST_SEEK_FLAG_FLUSH,
                        GST_SEEK_TYPE_SET, (gint64)(seek * GST_SECOND),
                        GST_SEEK_TYPE_NONE, GST_CLOCK_TIME_NONE));

    gst->priv->seeking = TRUE;
}

void respite_gst_set_volume(RespiteGst *gst, gdouble volume) {
...
void respite_gst_next_dvd_chapter(RespiteGst *gst) {
    respite_gst_change_dvd_chapter(gst, 1);
}
...
void respite_gst_set_dvd_chapter(RespiteGst *gst, gint chapter) {
    GstFormat format = gst_format_get_by_nick("chapter");
...
void respite_gst_seek_cdda(RespiteGst *gst, guint track_num) {
    gint current_track;

    current_track = respite_gst_get_current_cdda_track(gst);

    respite_gst_change_cdda_track(gst, (gint)track_num - current_track - 1);
}

gint respite_gst_get_current_cdda_track(RespiteGst *gst) {
    GstFormat format = gst_format_get_by_nick("track");
    gint64 pos;
    gint ret_val = 1;
...
void
respite_gst_set_cursor_visible(RespiteGst *gst, gboolean visible) {
    if ( visible ) {
        ...
        respite_window_invisible_cursor(gtk_widget_get_window(GTK_WIDGET(gst)));
    }
}

GList *
gst_get_lang_list_for_type(RespiteGst * gst, const gchar * type_name) {
    GList *ret = NULL;
    gint num = 1;
...
gboolean
gst_get_has_vis(RespiteGst *gst) {
    gboolean has_vis;
...
gboolean
gst_get_has_video(RespiteGst *gst) {
    gboolean playing_video;
...
void
gst_set_current_audio_track(RespiteGst *gst, gint track_no) {
    g_object_set(G_OBJECT(gst->priv->playbin), "current-audio", track_no, NULL);
}

void
gst_set_current_subtitle_track(RespiteGst *gst, gint track_no) {
    gchar *uri, *sub;
    gint flags;
    ...
    if ( gst->priv->use_custom_subtitles == TRUE )
        ...
        sub = (gchar*) respite_get_subtitle_path(uri);
...
const RespiteStream     *respite_gst_get_stream(RespiteGst *gst) {
    g_return_val_if_fail(RESPITE_IS_GST(gst), NULL);
