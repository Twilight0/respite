...
static void respite_open_location_finalize(GObject *object);

struct RespiteOpenLocation {
    GObject             parent;
    GtkWidget          *dialog;
    GtkWidget          *entry;
};

struct RespiteOpenLocationClass {
    GObjectClass    parent_class;

    void(*location_opened)(RespiteOpenLocation *self,
                                         const gchar *address);
};
...
G_DEFINE_TYPE(RespiteOpenLocation, respite_open_location, G_TYPE_OBJECT)

/* Callback for the open button which passes on the location and closes the dialog */
static void
respite_open_location_response_cb(GtkDialog *dialog, gint response_id, RespiteOpenLocation *self) {
    if ( response_id == 0 )
        return;
...
static void
respite_open_location_close(RespiteOpenLocation *self)
{
    gtk_widget_destroy(GTK_WIDGET(self->dialog));
}

static void
respite_open_location_open(RespiteOpenLocation *self)
{
...
static GtkTreeModel *
respite_open_location_get_completion_model(void) {
    GtkListStore *store;
    GtkTreeIter iter;
    gchar **lines = NULL;
    guint i;
...
static void
respite_open_location_class_init(RespiteOpenLocationClass *klass) {
    GObjectClass *object_class = G_OBJECT_CLASS(klass);

    object_class->finalize = respite_open_location_finalize;

    signals[LOCATION_OPENED] =
        g_signal_new("location-opened",
                     RESPITE_TYPE_OPEN_LOCATION,
                     G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(RespiteOpenLocationClass, location_opened),
                     NULL, NULL,
...
static void
respite_open_location_init(RespiteOpenLocation *self) {
}

static void
respite_open_location_finalize(GObject *object) {
    G_OBJECT_CLASS(respite_open_location_parent_class)->finalize(object);
}

/* Clear the location history */
static void
respite_open_location_clear_history(GtkTreeModel *model) {
    respite_clear_history_file();
    gtk_list_store_clear(GTK_LIST_STORE(model));
}

/* Main function to open the "open location" dialog */
RespiteOpenLocation *respite_open_location(GtkWidget *parent) {
    RespiteOpenLocation *self;
    GtkTreeModel *model;
    GtkBuilder *builder;
    ...
    self = g_object_new(RESPITE_TYPE_OPEN_LOCATION, NULL);

    builder = respite_builder_new_from_resource("/org/gnome/respite/open-location.ui");
    ...
    model = respite_open_location_get_completion_model();
    ...
    g_signal_connect_swapped(gtk_builder_get_object(builder, "clear-history"), "clicked",
                              G_CALLBACK(respite_open_location_clear_history), model);
    ...
    g_signal_connect_swapped(cancel, "clicked",
                      G_CALLBACK(respite_open_location_close), self);

    g_signal_connect_swapped(open, "clicked",
                      G_CALLBACK(respite_open_location_open), self);

    g_signal_connect(self->dialog, "delete-event",
                      G_CALLBACK(gtk_widget_destroy), NULL);

    g_signal_connect(self->dialog, "response",
                      G_CALLBACK(respite_open_location_response_cb), self);
