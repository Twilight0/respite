 ...
 */
gint
thunar_file_compare_by_name(RespiteFile *file_a,
                            RespiteFile *file_b,
    ...
    ap = respite_file_get_display_name(file_a);
    bp = respite_file_get_display_name(file_b);
    ...
    if (G_UNLIKELY(g_ascii_isdigit(ac) || g_ascii_isdigit(bc))) {
         ...
         */
        if (ap > respite_file_get_display_name (file_a) && bp > respite_file_get_display_name (file_b)
...
gchar *
respite_get_name_without_extension(const gchar *name) {
    guint len, suffix;
    gchar *ret;
...
static gchar *
respite_get_subtitle_in_dir(const gchar *dir_path, const gchar *file) {
    gchar *sub_path = NULL;
    gchar *file_no_ext;
    guint i;

    file_no_ext = respite_get_name_without_extension(file);
...
gchar *respite_get_subtitle_path(const gchar *uri) {
    GFile *file, *parent;
    GFileInfo *info;
    GError *error = NULL;
    gchar *path;
    ...
    ret = respite_get_subtitle_in_dir(path, file_name);
...
gboolean
respite_is_uri_disc(const gchar *uri) {
...
GdkPixbuf *respite_icon_load(const gchar *icon_name, gint size) {
    GdkPixbuf *pix = NULL;
    GError *error = NULL;
...
void respite_get_media_files(GtkFileFilter *filter, const gchar *path, gboolean recursive, GSList **list) {
    ...
    if (g_file_test(path, G_FILE_TEST_IS_REGULAR)) {
        file = respite_file_new(path);
        if ( respite_file_filter (playlist_filter, file) &&
             respite_pl_parser_guess_format_from_extension(path) != RESPITE_PL_FORMAT_UNKNOWN ) {
            playlist = respite_pl_parser_parse_from_file_by_extension(path);
            g_object_unref(file);
            if (playlist) {
                *list = g_slist_concat(*list, playlist);
            }
        } else if (respite_file_filter(filter, file)) {
        ...
        while ((name = g_dir_read_name(dir))) {
            ...
            if (g_file_test(path_internal, G_FILE_TEST_IS_DIR) && recursive) {
                respite_get_media_files(filter, path_internal, TRUE, list);
            } else if ( g_file_test(path_internal, G_FILE_TEST_IS_REGULAR) ) {
                file = respite_file_new(path_internal);
                if (respite_file_filter (playlist_filter, file) &&
                     respite_pl_parser_guess_format_from_extension(path) != RESPITE_PL_FORMAT_UNKNOWN) {
                    playlist = respite_pl_parser_parse_from_file_by_extension(path_internal);
                    g_object_unref(file);
                    if (playlist) {
                        *list = g_slist_concat(*list, playlist);
                    }
...
/***
 * FIXME, respite_device_has_cdda and respite_guess_uri_from_mount
 *        have common code with respite-disc.c
 ***/


gboolean
respite_device_has_cdda(const gchar *device) {
...
gchar *
respite_guess_uri_from_mount(GMount *mount) {
    GFile *file;
    gchar *uri = NULL;
...
gchar *
respite_get_uri_from_unix_device(const gchar *device) {
    ...
    /*Check for cdda */
    if (respite_device_has_cdda(device)) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (g_drive_can_eject(drive) && g_drive_has_media(drive)) {
            ...
            if (g_strcmp0(unix_device, device) == 0) {
                ...
                if ( mount ) {
                    uri = respite_guess_uri_from_mount(mount);
...
/**
 * respite_taglibc_get_media_length:
 *
 * @RespiteFile: a RespiteFile
 *
 * Returns: the length of the media only if the file is a local
 *          media file.
 **/
gchar *respite_taglibc_get_media_length(RespiteFile *file) {
    #ifdef HAVE_TAGLIBC

    TagLib_File *tag_file;

    if (g_str_has_prefix(respite_file_get_uri(file), "file:/")) {
        tag_file = taglib_file_new(respite_file_get_file_name(file));
        ...
        if ( tag_file ) {
            ...
            if (length != 0)
                return respite_format_media_length (length);
        }
    }
    #endif /* HAVE_TAGLIBC */
