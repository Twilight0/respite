...
respite_session_bus_get(void) {
    return dbus_g_connection_get_connection (respite_g_session_bus_get ());
}

DBusGConnection *
respite_g_session_bus_get(void) {
...
DBusGProxy *
respite_get_proxy(const gchar *path, const gchar *iface) {
    DBusGConnection *bus;
    DBusGProxy *proxy;

    bus = respite_g_session_bus_get();
...
gboolean
respite_dbus_name_has_owner(const gchar *name) {
    DBusConnection *bus;
    DBusError error;
    gboolean ret;

    bus = respite_session_bus_get();
...
gboolean respite_dbus_register_name(const gchar *name) {
    DBusConnection *bus;
    DBusError error;
    int ret;
...
gboolean respite_dbus_release_name(const gchar *name) {
    DBusConnection *bus;
    DBusError error;
    int ret;
