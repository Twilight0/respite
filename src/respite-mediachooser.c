 ...
 */
void    respite_media_chooser_add_clicked(GtkWidget *widget,
                                             RespiteMediaChooser *chooser);

void    respite_media_chooser_close_clicked(GtkWidget *widget,
                                             RespiteMediaChooser *chooser);

void    respite_media_chooser_destroy_cb(GtkWidget *widget,
                                             RespiteMediaChooser *chooser);
...
struct RespiteMediaChooserClass {
    GObjectClass        parent_class;

    void(*media_files_opened)(RespiteMediaChooser *chooser,
                                                 GSList *list);

    void(*iso_opened)(RespiteMediaChooser *chooser,
                                                 gchar *filename);
};
...
G_DEFINE_TYPE(RespiteMediaChooser, respite_media_chooser, G_TYPE_OBJECT)

void
media_chooser_folder_changed_cb(GtkWidget *widget, RespiteMediaChooser *chooser) {
    gchar *folder;
...
static void
respite_media_chooser_add(RespiteMediaChooser *chooser, GtkWidget *file_chooser) {
    ...
    for (i = 0; i < len; i++) {
        ...
        respite_get_media_files(filter, file, scan_recursive, &media_files);
    }
...
static gboolean
respite_media_chooser_add_idle(gpointer data) {
    RespiteMediaChooser *chooser;
    GtkWidget *file_chooser;

    chooser = RESPITE_MEDIA_CHOOSER(data);

    file_chooser = GTK_WIDGET(g_object_get_data(G_OBJECT(chooser), "file-chooser"));

    respite_media_chooser_add(chooser, file_chooser);
...
static void
respite_media_chooser_open(RespiteMediaChooser *chooser) {
    respite_window_busy_cursor(gtk_widget_get_window(GTK_WIDGET(chooser->window)));

    gtk_widget_show(chooser->spinner);

    g_idle_add((GSourceFunc) respite_media_chooser_add_idle, chooser);
}

void respite_media_chooser_add_clicked(GtkWidget *widget, RespiteMediaChooser *chooser) {
    respite_media_chooser_open(chooser);
}

/* Destroy the widget when close is clicked */
void respite_media_chooser_close_clicked(GtkWidget *widget, RespiteMediaChooser *chooser) {
    gtk_widget_destroy(chooser->window);
}

void respite_media_chooser_destroy_cb(GtkWidget *widget, RespiteMediaChooser *chooser) {
    g_object_unref(chooser);
}

void media_chooser_file_activate_cb(GtkFileChooser *filechooser, RespiteMediaChooser *chooser) {
    respite_media_chooser_open(chooser);
}

static void
respite_media_chooser_open_internal(RespiteMediaChooser *media_chooser) {
    GtkWidget      *file_chooser;
    GtkBuilder     *builder;
    GtkWidget      *recursive;
    GtkWidget      *box;
    ...
    builder = respite_builder_new_from_resource("/org/gnome/respite/mediachooser.ui");
...
static void
respite_media_chooser_finalize(GObject *object) {
    RespiteMediaChooser *chooser;

    chooser = RESPITE_MEDIA_CHOOSER(object);
    ...
    G_OBJECT_CLASS(respite_media_chooser_parent_class)->finalize(object);
}

static void
respite_media_chooser_class_init(RespiteMediaChooserClass *klass) {
    GObjectClass *object_class = G_OBJECT_CLASS(klass);

    signals[MEDIA_FILES_OPENED] =
        g_signal_new("media-files-opened",
                     RESPITE_TYPE_MEDIA_CHOOSER,
                     G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(RespiteMediaChooserClass, media_files_opened),
                     NULL, NULL,
    ...
    signals[ISO_OPENED] =
        g_signal_new("iso-opened",
                     RESPITE_TYPE_MEDIA_CHOOSER,
                     G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(RespiteMediaChooserClass, iso_opened),
                     NULL, NULL,
                     g_cclosure_marshal_VOID__POINTER,
                     G_TYPE_NONE, 1,
                     G_TYPE_POINTER);

    object_class->finalize = respite_media_chooser_finalize;
}

static void
respite_media_chooser_init(RespiteMediaChooser *chooser) {
    chooser->conf = respite_conf_new();
}

RespiteMediaChooser *respite_media_chooser_open_local(GtkWidget *parent) {
    RespiteMediaChooser *chooser;

    chooser = g_object_new(RESPITE_TYPE_MEDIA_CHOOSER, NULL);

    respite_media_chooser_open_internal(chooser);
